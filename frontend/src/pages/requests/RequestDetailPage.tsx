...
export default function RequestDetailPage() {
  ...
  const { data: request, isLoading, error } = useQuery({
    queryKey: ['request', id],
    queryFn: async () => (await api.get<{ data: RequestDetail }>(`/requests/${id}`)).data.data,
  });
  ...
  if (error || !request) {
    return <p className="text-sm text-danger">Couldn't load this request — it may not exist, or you may not have access to it.</p>;
  }

  const isOwner = request.requesterId === currentUser?.id;
  const canCancel = isOwner && ['DRAFT', 'SUBMITTED', 'PENDING_APPROVAL'].includes(request.status);
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        actions={<Badge tone={statusTone(request.status)}>{request.status.replace('_', ' ')}</Badge>}
      />
        ...
        <Card>
          ...
          {(submitMutation.isError || cancelMutation.isError) && (
            <p className="px-5 pb-4 text-xs text-danger">
              {((submitMutation.error ?? cancelMutation.error) as any)?.response?.data?.message ?? 'Something went wrong.'}
            </p>
          )}
        </Card>

        <Card>
          <CardHeader className="text-sm font-medium">Approval timeline</CardHeader>
          <CardBody>
            {!request.workflowInstance && <p className="text-sm text-muted">Not submitted yet.</p>}
            {request.workflowInstance && (
              <>
                {request.workflowInstance.status === 'PENDING' && currentStepName && (
                  <div className="mb-4 flex items-center justify-between rounded-md bg-accent/10 px-3 py-2 text-sm text-accent">
                    ...
                    <div className="flex gap-1.5">
                      <button onClick={() => approveMutation.mutate()} className="rounded p-1 hover:bg-white/50" title="Approve">
                        <Check className="h-4 w-4" />
                      </button>
                      <button onClick={() => rejectMutation.mutate()} className="rounded p-1 hover:bg-white/50" title="Reject">
                  ...
                  {request.workflowInstance.history.map((h) => (
                    <li key={h.id} className="flex items-start gap-3 text-sm">
                      ...
                      <div>
                        ...
                        {h.comment && <p className="text-xs text-muted">"{h.comment}"</p>}
